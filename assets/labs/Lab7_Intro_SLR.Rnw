...
\newline
\newline
\textbf{Note:} You need to access this function using \hlkwd{source()}. The \emph{plot\_ss.R} file that contains the code for this function can be found in the lab source folder in module OpenIntro3. To access it, change the directory path to where \emph{plot\_ss.R} is saved on your device. If you saved the module to your desktop, for example, all you would need to change is \emph{nunezsa} to your username in the following source code. 

<<plotss-atbats-runs, eval=FALSE>>=
source("/Users/nunezsa/Desktop/OpenIntro3/source/labs/plot_ss.R")
...
\begin{exercise}
Run this code several more times trying to minimize the sum of squares each time. What is the smallest sum of squares you can obtain? How does it compare to your neighbors?
\end{exercise}
...
\begin{enumerate}
\item Linearity: You already checked if the relationship between CO content and amount of tar is linear using a scatterplot. We should also verify this condition with a plot of the residuals vs. \hlkwd{tar}.  Recall that any code following a \hlcom{\#} is intended to be a comment that helps understand the code but is ignored by R.

<<residuals, eval=FALSE>>=
plot(m1$residuals ~ cigs$tar)
