...
library(ggplot2)
qplot(hip, weight, data=body)
qplot(chest, weight, data=body)
qplot(wrist.diam, weight, data=body)
qplot(thigh, weight, data=body, color=gender)
@
...
lm_gender_thigh <- lm(weight ~ gender + thigh, data=body)
...
lm_full <- lm(weight ~ shoulder + chest + waist + navel + hip + thigh + bicep + forearm + knee + calf, data = body)
summary(lm_full)
@
