...
\begin{exercise}
 Define a class of models that you will explore. What predictor variables will you use? What combinations will you evaluate? What, if any, transformations or "curvilinearities" will you explore? What metric will you use to pick the best model out of this subset of models?
\end{exercise}
...
\begin{exercise}
For the chosen model, calculate Cook's distance without using the {\tt cooks.distance()} function. Plot the Cook's distance values. Do any points look worrisomely influential?
\end{exercise}
