...
\begin{frame}{Checking model structure}

\bi
    \myitem You can plot residuals against each of the predictors, or plot outcomes against predictors
	\myitem Keep in mind the MLR uses adjusted relationships; scatterplots don't show that adjustment
	\myitem Adjusted variable plots (partial regression plots, added variable plots) can be useful
\ei
...
\begin{frame}{Adjusted variable plots}
